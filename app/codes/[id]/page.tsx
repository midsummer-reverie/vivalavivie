...
export default function CodeDetail() {
  ...
  // 🌟 [เพิ่มใหม่] State สำหรับ Live Customizer
  const [customText, setCustomText] = useState("ข้อความตัวอย่าง...");
  ...
  // 🌟 [เพิ่มใหม่] ฟังก์ชันแทรก BBCode ตรงจุดที่เคอร์เซอร์อยู่
  const insertBBCode = (openTag: string, closeTag: string = '') => {
    ...
    setCustomText(before + openTag + selected + closeTag + after);
    
    // โฟกัสกลับไปที่เดิมหลังแทรกโค้ด
    setTimeout(() => {
      el.focus();
  ...
  // 🌟 [เพิ่มใหม่] ฟังก์ชันแปลง BBCode เป็น HTML
  ...
  const getRenderedHtml = () => {
    ...
    // 1. นำค่าจาก Variation มาแทนที่ (ถ้ามี)
    ...
    // 🌟 2. นำค่าจาก Live Customizer มาแทนที่ (ทำงานทับลงไปอีกที)
    finalHtml = finalHtml.split('**CUSTOM_TEXT**').join(parseBBCode(customText));
    finalHtml = finalHtml.split('**CUSTOM_COLOR**').join(customColor);
    finalHtml = finalHtml.split('**BOX_DISPLAY**').join(showBox ? 'block' : 'none');
  ...
  return (
    ...
    >
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .glass-card { background: rgba(255,255,255,0.5); border: 1px solid var(--glass-border); border-radius: var(--radius-lg); padding: 32px; backdrop-filter: blur(16px); box-shadow: 0 10px 40px rgba(139, 92, 246, 0.08); margin-bottom: 30px; }
        ...
        .lock-input { width: 100%; max-width: 300px; padding: 14px; border-radius: var(--radius-md); border: 1px solid var(--theme-primary); background: rgba(255,255,255,0.9); font-size: 1rem; text-align: center; margin-bottom: 16px; outline: none; }
        ...
        .var-btn { display: flex; align-items: center; gap: 8px; background: rgba(255,255,255,0.6); border: 1px solid var(--glass-border); padding: 8px 16px; border-radius: 20px; cursor: pointer; font-weight: 600; color: #4c1d95; font-size: 0.9rem; transition: 0.2s; }
        ...
        /* 🌟 CSS สำหรับ Editor Panel */
        .editor-panel { background: rgba(255,255,255,0.7); border: 1px dashed #a855f7; border-radius: var(--radius-md); padding: 20px; margin-bottom: 24px; display: flex; flex-direction: column; gap: 16px; }
        .editor-header { font-weight: 700; color: #4c1d95; display: flex; align-items: center; gap: 8px; }
        .bb-toolbar { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 8px; }
        .bb-btn { background: #fff; border: 1px solid #d8b4fe; border-radius: 6px; padding: 4px 12px; font-size: 0.9rem; font-weight: 600; color: #6b21a8; cursor: pointer; transition: 0.2s; }
        .bb-btn:hover { background: #f3e8ff; border-color: #a855f7; }
        .custom-textarea { width: 100%; min-height: 100px; padding: 12px; border-radius: 8px; border: 1px solid #d8b4fe; background: rgba(255,255,255,0.9); font-family: inherit; resize: vertical; outline: none; }
        .custom-textarea:focus { box-shadow: 0 0 0 3px rgba(216, 180, 254, 0.4); }
        .editor-row { display: flex; gap: 24px; flex-wrap: wrap; }
        .editor-item { display: flex; align-items: center; gap: 8px; font-weight: 600; color: #4c1d95; font-size: 0.95rem; }
          ...
          {!isUnlocked ? (
            <div className="lock-screen">
              ...
              <form onSubmit={handleUnlock}>
                <input type="password" className="lock-input" placeholder="Enter password..." value={passwordInput} onChange={(e) => { setPasswordInput(e.target.value); setPasswordError(false); }} autoFocus />
            ...
            <>
              ...
              {/* 🌟 ส่วน Editor สำหรับปรับแต่ง (Live Customizer) */}
              <div className="editor-panel">
                <div className="editor-header">🎨 ปรับแต่งโค้ด (Live Customizer)</div>
                
                <div>
                  <div className="bb-toolbar">
                    <button className="bb-btn" onClick={() => insertBBCode('[b]', '[/b]')}>B</button>
                    <button className="bb-btn" onClick={() => insertBBCode('[i]', '[/i]')}>I</button>
                    <button className="bb-btn" onClick={() => insertBBCode('[u]', '[/u]')}>U</button>
                    <button className="bb-btn" onClick={() => insertBBCode('[color=#ff0000]', '[/color]')}>Color</button>
                  </div>
                  <textarea 
                    ref={textareaRef}
                ...
                <div className="editor-row">
                  <label className="editor-item">
                    สีธีมหลัก:
                    <input type="color" value={customColor} onChange={(e) => setCustomColor(e.target.value)} style={{ cursor: 'pointer' }} />
                  </label>
                  
                  <label className="editor-item">
                    <input type="checkbox" checked={showBox} onChange={(e) => setShowBox(e.target.checked)} style={{ cursor: 'pointer', width: '18px', height: '18px' }} />
                    แสดงกล่องเสริม (Extra Box)
                  </label>
                </div>
              </div>
